...
static void drop_old(pa_smoother *s, uint64_t x) {
...
static double avg_gradient(pa_smoother *s, uint64_t x) {
    unsigned i, j, c = 0;
...
void pa_smoother_set_time_offset(pa_smoother *s, uint64_t offset) {
    pa_assert(s);
